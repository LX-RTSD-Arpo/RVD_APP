                 ...
                 uint16_t *max_queue)
{
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        if (key && value)
        {
            ...
            else if (strcmp(key, "MAX_QUEUE") == 0)
                *max_queue = atoi(value);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    cb1.qbuffer = malloc(max_queue * sizeof(unsigned char *));
    for (int i = 0; i < max_queue; ++i)
    {
        cb1.qbuffer[i] = malloc(BUF_SIZ * sizeof(unsigned char));
    }
