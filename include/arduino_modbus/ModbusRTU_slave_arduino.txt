...
const byte ledPins[2] = {31, 33};
//const byte buttonPins[2] = {3, 2};
const byte vPins[2] = {A0, A1};
...
void setup() {
  ...
  pinMode(vPins[0], INPUT);
  pinMode(vPins[1], INPUT);
...
void loop() {
  ...
  //volt[0] = analogRead(vPins[0]);
  //volt[1] = analogRead(vPins[1]);
  holdingRegisters[0] = map(analogRead(vPins[0]), 0, 1023, 0, 5);
  holdingRegisters[1] = map(analogRead(vPins[1]), 0, 1023, 0, 5);
